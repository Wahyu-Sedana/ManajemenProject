<x-filament-panels::page>
        ...
        <x-filament::section>
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <h2 class="text-lg font-medium text-gray-900 dark:text-white">
                    {{ $selectedProject ? $selectedProject->name : __('board.select_project') }}
                </h2>

                <div class="w-full sm:w-auto">
                    <x-filament::input.wrapper>
                        <x-filament::input.select wire:model.live="selectedProjectId" class="w-full">
                            <option value="">{{ __('board.select_project') }}</option>
    ...
    @if ($selectedProject)
        <div x-data="dragDropHandler()" x-init="init()" @ticket-moved.window="init()" @ticket-updated.window="init()"
            @refresh-board.window="init()" class="relative overflow-x-auto pb-6" id="board-container">
            {{-- Scroll indicators --}}
            <div class="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-gray-100 dark:from-gray-800 to-transparent pointer-events-none z-10"
                x-data="{ visible: false }" x-init="$nextTick(() => {
                    const container = document.getElementById('board-container');
                    container.addEventListener('scroll', () => {
                        visible = container.scrollLeft > 20;
                    });
                })" x-show="visible" x-transition.opacity></div>
            ...
            <div class="inline-flex gap-4 pb-2 min-w-full">
                @foreach ($ticketStatuses as $status)
                    <div class="status-column rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col bg-gray-50 dark:bg-gray-900"
                        ...
                        <div class="px-4 py-3 rounded-t-xl border-b border-gray-200 dark:border-gray-700"
                            ...
                            <h3 class="font-medium flex items-center justify-between"
                                style="color: white; text-shadow: 0px 0px 1px rgba(0,0,0,0.5);">
                            ...
                            @foreach ($status->tickets as $ticket)
                                <div class="ticket-card bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 cursor-move"
                                    ...
                                    <div class="flex justify-between items-center mb-2">
                                        ...
                                        @if ($ticket->due_date)
                                            <span
                                                class="text-xs px-1.5 py-0.5 rounded whitespace-nowrap {{ $ticket->due_date->isPast() ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' : 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300' }}">
                                    ...
                                    <div class="flex justify-between items-center mt-2">
                                        @if ($ticket->assignee)
                                            <div
                                                class="inline-flex items-center px-2 py-1 rounded-full bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300 gap-1 truncate">
                                                <span
                                                    class="w-4 h-4 rounded-full bg-primary-500 flex items-center justify-center text-xs text-white flex-shrink-0">
                                                    {{ substr($ticket->assignee->name, 0, 1) }}
                                                </span>
                                                <span
                                                    class="text-xs font-medium truncate">{{ $ticket->assignee->name }}</span>
                                            </div>
                                        @else
                                            <div
                                                class="inline-flex items-center px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-400">
                                                ...
                                                <span class="text-xs font-medium">{{ __('board.unassigned') }}</span>
                                            </div>
                                        @endif

                                        <button type="button" wire:click="showTicketDetails({{ $ticket->id }})"
    ...
    @else
        <div class="flex flex-col items-center justify-center h-64 text-gray-500 dark:text-gray-400 gap-4">
            ...
            <h2 class="text-xl font-medium text-gray-600 dark:text-gray-300">
                {{ __('board.no_project_heading') }}
            </h2>
            <p class="text-sm text-gray-500 dark:text-gray-400">
                {{ __('board.no_project_description') }}
            </p>
        </div>
    @endif
